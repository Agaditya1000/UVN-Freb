...
import { DollarSign, Landmark, Building2, TrendingUp } from 'lucide-react';
import Footer from '../../components/Footer';

const MetricCard = ({ title, amount, icon, desc, symbol, isPositive }) => (
  <div className="group p-6 bg-gradient-to-br from-bg to-surface border border-border rounded-3xl shadow-md hover:shadow-2xl hover:-translate-y-1 transition-all duration-500 relative overflow-hidden">
    ...
    <div className="flex justify-between items-start mb-4 relative z-10">
      <div>
        <p className="text-text-secondary text-[11px] font-bold uppercase tracking-widest mb-2">{title}</p>
        ...
        <p className="text-xs text-text-secondary mt-2 leading-relaxed max-w-[220px]">{desc}</p>
      </div>
  ...
  if (!activeBusiness) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-bg text-text text-center">
        ...
        <p className="text-text-secondary">Please select or create a business profile from the navigation menu.</p>
      </div>
    );
  }
