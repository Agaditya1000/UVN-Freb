...
const ChartOfAccounts = () => {
  ...
  const handleAdd = (e) => {
    ...
    if(newAccntCode && newAccntName) {
      addAccount({ id: newAccntCode, name: newAccntName, category: newAccntCat });
      setShowModal(false);
      setNewAccntCode('');
      setNewAccntName('');
    }
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-naviBlue text-xs uppercase tracking-heading text-grayText border-b-[1.75px] border-borderDark">
                      <th className="p-4 font-medium w-32 border-r-[1.75px] border-borderDark">Code</th>
                      <th className="p-4 font-medium border-r-[1.75px] border-borderDark">Account Name</th>
                      <th className="p-4 font-medium text-right w-48">Current Balance</th>
                    </tr>
                  </thead>
                  <tbody>
                    {accs.map(acc => (
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
          <div className="panel p-6 sm:p-8 w-full max-w-md anim-fade-up">
             ...
             <form onSubmit={handleAdd} className="space-y-5">
               <div>
                  <label className="block text-sm font-medium tracking-heading mb-1.5 text-lightWhite">Account Code</label>
               ...
               <div>
                  ...
                  <select value={newAccntCat} onChange={e=>setNewAccntCat(e.target.value)} className="w-full bg-black border-[1.75px] border-borderDark p-3 text-white focus:outline-none focus:border-accent font-light appearance-none">
